...
async def gcast(event):
    if xx := event.pattern_match.group(1):
        ...
        return await eor(
            event, "`Berikan bacotan lu ke Globally Broadcast atau balas pesan..`"
        )
    kk = await event.eor("`📶Sabar ya ngentot lagi ngirim bacotan lu asu...`")
    er = 0
    done = 0
    err = ""
        ...
        if x.is_group:
            ...
            if chat not in chat_blacklist and chat not in NOSPAM_CHAT:
                try:
                    ...
                    er += 1
    await kk.edit(
        f"**✅Berhasil ngirim bacotan lu ke grup keren : `{done}` Grup.\n❎Gagal ya ngentot Ke : `{er}` Grup.**"
    )
